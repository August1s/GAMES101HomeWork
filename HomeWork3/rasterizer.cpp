...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
    ...
    // Homework2的遗留问题，boundingbox不够大，不能直接四舍五入。
    int left, right, top, bottom; 
    right = (int)(fmax(fmax(v[0].x(), v[1].x()), v[2].x())) + 1;
    left = (int)(fmin(fmin(v[0].x(), v[1].x()), v[2].x())) - 1;
    top = (int)(fmax(fmax(v[0].y(), v[1].y()), v[2].y())) + 1;
    bottom = (int)(fmin(fmin(v[0].y(), v[1].y()), v[2].y())) - 1;
    ...
    for (int x = left; x < right; x++)
    {
        for (int y = bottom; y < top; y++)
        {
            if (insideTriangle(x + 0.5, y + 0.5, t.v))
            {
                ...
                float zp = (alpha * v[0].z() + beta * v[1].z() + gamma * v[2].z()) / weight;
